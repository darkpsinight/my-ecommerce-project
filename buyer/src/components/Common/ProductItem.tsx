...
const ProductItem = ({ item }: { item: Product }) => {
  ...
  const isInWishlist = useSelector((state: RootState) => 
    selectIsItemInWishlist(state, item.id)
  );
  
  // Check if wishlist is loading
      ...
      <div className="relative overflow-hidden rounded-lg bg-[#F6F7FB] mb-4 w-full h-[270px]">
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0 z-20">
          ...
          <button
            ...
            className={`flex items-center justify-center w-9 h-9 rounded-[5px] shadow-1 ease-out duration-200 relative z-20 disabled:cursor-not-allowed disabled:opacity-70 ${
              isInWishlist 
