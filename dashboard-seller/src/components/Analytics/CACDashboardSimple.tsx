...
const CACDashboardSimple: React.FC<CACDashboardProps> = ({ 
  timeRange = '30d', 
  onTimeRangeChange 
}) => {
            ...
            <CardContent>
              <TableContainer>
                <Table>
                  ...
                  <TableBody>
                    {cacData.cacByChannel.map((row) => (
                      <TableRow key={row.channel}>
                        <TableCell component="th" scope="row">
                          {getChannelLabel(row.channel)}
                        </TableCell>
                        <TableCell align="right">{formatCurrency(row.totalSpend)}</TableCell>
...
export default CACDashboardSimple;