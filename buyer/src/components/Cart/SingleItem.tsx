    ...
    <div className={`group relative flex flex-col lg:flex-row items-start lg:items-center gap-6 py-8 px-6 transition-all duration-300 hover:bg-blue-light-5/30 cart-item-hover ${isUpdating ? 'opacity-60' : ''}`}>
      ...
      <div className="flex-grow flex flex-col sm:flex-row items-start sm:items-center gap-6 min-w-0">
        <Link href={`/shop-details?id=${item.id}`} className="shrink-0">
          <div className="relative w-24 h-24 sm:w-32 sm:h-32 bg-gradient-to-br from-gray-1 to-gray-2 rounded-2xl overflow-hidden shadow-1">
            ...
            {/* Quantity Badge */}
            {item.quantity > 1 && (
              <div className="absolute -top-2 -right-2 w-8 h-8 bg-blue rounded-full flex items-center justify-center shadow-lg border-2 border-white">
            ...
            <div className="flex items-baseline gap-4">
              ...
              <div className="bg-white rounded-xl px-3 py-2 shadow-1">
                <span className="text-sm font-medium text-gray-6">
                  Subtotal: <span className="font-bold text-gray-7">${formatPrice(multiplyCurrency(item.discountedPrice, item.quantity))}</span>
                </span>
              </div>
            </div>
      ...
      <div className="flex items-center justify-center lg:self-center">
        <button
          ...
          className="group/remove flex flex-col items-center justify-center gap-2 w-20 h-20 rounded-2xl bg-white border-2 border-gray-3 text-gray-5 transition-all duration-300 hover:border-red hover:bg-red-light-6 hover:text-red disabled:opacity-50 disabled:cursor-not-allowed shadow-1 hover:shadow-2"
        >
          {isUpdating ? (
            <div className="w-6 h-6 border-2 border-red border-t-transparent rounded-full animate-spin"></div>
          ) : (
            <svg
