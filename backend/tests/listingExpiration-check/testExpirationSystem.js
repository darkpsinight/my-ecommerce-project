...
const { updateExpiredListings } = require('../../jobs/listingExpirationCron');
...
const createTestListing = async (expirationDate) => {
  ...
  // Add a test code
  listing.addCodes(['TEST-CODE-12345']);
...
const runTests = async () => {
  try {
    ...
    const result = await updateExpiredListings(mockFastify);
    ...
    const wasUpdated = checkAndUpdateExpiredStatus(updatedListing);
