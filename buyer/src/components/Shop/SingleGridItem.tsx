...
const SingleGridItem = ({ 
  ...
  return (
    <div ref={elementRef} className="group relative" data-product-id={item.id}>
      <Link 
      ...
      <div className="relative overflow-hidden rounded-lg bg-white shadow-1 mb-3 w-full h-[220px]">
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-1.5 pb-3 ease-linear duration-200 group-hover:translate-y-0 z-20">
          ...
          <button
            ...
            id="favOne"
            className={`flex items-center justify-center w-7 h-7 rounded-[4px] shadow-1 ease-out duration-200 relative z-20 disabled:cursor-not-allowed disabled:opacity-70 ${
              isInWishlist 
                ? 'text-white bg-red hover:bg-red-dark' 
                : 'text-dark bg-white hover:text-red'
            }`}
          >
            {isWishlistLoading ? (
              // Loading spinner
